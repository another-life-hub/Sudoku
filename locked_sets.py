...
def call_locked_set(dim):
    def locked_set(rc, bp):
        """Вывод на экран результата поиска замкнутых множеств
...
def call_hidden_set(dim):
    def hidden_set(rn, cn, bn):
        """Вывод на экран результата поиска скрытых множеств
           размерности dim >=2 и передача данных основному модулю"""
        if dim == 2: pr1 = 'пара'
        if dim == 3: pr1 = 'тройка'
        if dim == 4: pr1 = 'четвёрка'
...
def call_fish(dim):
    def fish(rn, cn):
        """Вывод на экран результата поиска решёток без брака
           размерности dim >=2 и передача данных основному модулю"""
        res = ()
